...
int32_t rvv_vwmul_e8_m1(int8_t* x,  int8_t* w, int n) {
    // W (d,n) * x (n,)   -> xout (d,)
    ...
    if (n < block_size)
    {
        ...
        asm volatile("vmul.vv v0, v8, v12");
...
void matmul(float* xout, QuantizedTensor *x, QuantizedTensor *w, int n, int d) {
    ...
    for (i = 0; i < d; i++) {
        ...
        for (j = 0; j <= n - GS; j += GS) {
            #ifdef RVV
                ival = rvv_vwmul_e8_m1(&x->q[j], &w->q[in + j], GS);
            #else 
                for (int k = 0; k < GS; k++) {
...
void matmul_vifmm(float* xout, float* x, QuantizedTensor *w, int n, int d) {
    // W (d,n) @ x (n,) -> xout (d,)
    // by far the most amount of time is spent inside this little function
    // inputs to this function are both quantized
...
//
    ...
    for (i = 0; i < d; i++) {
        ...
        for (j = 0; j <= n - GS; j += GS) {
            for (int k = 0; k < GS; k++) {
                ival +=  x[j + k] * ((float) w->q[in + j + k]);
                // printf("%f ", ival);
            }
            val +=  ival * w->s[(in + j) / GS] ;
            ival = 0;
        }
...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(int l = 0; l < p->n_layers; l++) {
        ...
        matmul_vifmm(s->q, s->xb, w->wq + l, dim, dim);
        matmul_vifmm(s->k, s->xb, w->wk + l, dim, kv_dim);
        matmul_vifmm(s->v, s->xb, w->wv + l, dim, kv_dim);
        ...
        matmul_vifmm(s->xb2, s->xb, w->wo + l, dim, dim);
        ...
        matmul_vifmm(s->hb, s->xb, w->w1 + l, dim, hidden_dim);
        matmul_vifmm(s->hb2, s->xb, w->w3 + l, dim, hidden_dim);
        ...
        matmul_vifmm(s->xb, s->hb, w->w2 + l, hidden_dim, dim);
    ...
    matmul_vifmm(s->logits, x, w->wcls, dim, p->vocab_size);
    return s->logits;
}
