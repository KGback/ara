...
always_comb begin  // update olr_thd
    ...
    unique case (operand_eew_a)
        ...
        EW32: begin
            ...
            if (operand[0].e[7] && ((olr_thd_diff_e_fp32 + 7) < op0_diff_e_fp32)) begin 
                olr_thd_dynamic_d = operand[0];
            end

            // if ((olrthd_abs_comp_diff_e_fp32 + 7) < op1_abs_comp_diff_e_fp32 ) begin
            if (operand[1].e[7] && ((olr_thd_diff_e_fp32 + 7) < op1_diff_e_fp32)) begin        
                olr_thd_dynamic_d = operand[1];
            end
...
always_comb begin 
    ...
    if ( operand[1].e[7] && olr_thd_diff_e_fp32  < op1_diff_e_fp32 ) begin
        transfer_pack_o[1].transfer_type       = 0;
        transfer_pack_o[1].transfer_data       = operand[1].e;
    end else begin
    ...
    if ( operand[0].e[7] && olr_thd_diff_e_fp32  < op0_diff_e_fp32 ) begin
        transfer_pack_o[0].transfer_type       = 0;
        transfer_pack_o[0].transfer_data       = operand[0].e;
    end else begin
